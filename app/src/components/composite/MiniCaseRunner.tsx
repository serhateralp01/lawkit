...
import { useCaseSession } from "@/lib/case-engine";
 ...
 * `/vaka/$caseId` route'u ile birebir aynı engine'i kullanır (DRY).
 */
export function MiniCaseRunner() {
  const caseData = isHukuku001;
  const { session, node, chosenOption, pick, advance, reset } = useCaseSession(caseData);
  const recordAttempt = useGamificationStore((s) => s.recordAttempt);

  // Vaka outcome'a vardığında gamification'a yaz (idempotent — caseId+finishedAt).
  useEffect(() => {
    if (session.done) {
      const last = useGamificationStore.getState().attempts.at(-1);
      const justFinished =
        last?.caseId === session.caseId &&
        Date.now() - last.finishedAt < 1500;
      if (!justFinished) recordAttempt(session);
    }
    ...
    <div className="overflow-hidden rounded-2xl border border-line bg-surface-raised shadow-[var(--shadow-raised)]">
      ...
      <div className="grid gap-0 lg:grid-cols-[1fr_1.4fr_0.85fr]">
            ...
            {node?.kind === "decision" && node.options ? (
              ...
              >
                ...
                <ul className="space-y-2">
                  {node.options.map((o) => {
                    const isPicked = chosenOption?.id === o.id;
                    const isLocked = !!chosenOption;
                    const Icon =
                    ...
                    return (
                      <motion.li
                        ...
                        animate={isPicked ? { scale: [1, 1.02, 1] } : { scale: 1 }}
                      ...
                      >
                        <button
                          ...
                          className={cn(
                            ...
                            !isLocked && "border-line bg-surface-raised hover:border-indigo/40 hover:bg-indigo-soft/30",
                            isPicked && verdictColor,
                            isLocked && !isPicked && "border-line bg-surface-sunken/40 text-ink-3 opacity-60",
                          )}
                        >
            ...
            {node?.kind === "outcome" ? (
              ...
              >
                <motion.div
                  ...
                  className="inline-flex items-center gap-2 rounded-full bg-indigo-soft/50 px-3 py-1"
                >
        ...
        <aside className="space-y-6 border-line bg-surface-sunken/30 p-5 lg:border-l">
          ...
          {node?.kind === "decision" && !chosenOption ? (
            <HintLadder
              hint={{
